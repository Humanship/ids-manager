...
use crate::cnft::Noop;
use crate::cnft::SplAccountCompression;
...
pub mod base_ix {
    ...
    */
    ...
    pub fn create_collection<'a, 'b, 'c, 'info>(ctx: Context<'a, 'b, 'c, 'info, CreateCollection<'info>>, token_metadata:crate::cnft::TokenMetadata, vault_type:String) -> Result<()> {
      ...
      let mut uri = "https://arweave.net/".to_string();
      uri = uri + token_metadata.arweave.as_str();
...
pub fn fix_store_master(ctx: Context<FixStoreMaster>) -> Result<()> {
    
        let store = &mut ctx.accounts.store;
        let master = &mut ctx.accounts.master;
        

        if store.master == master.key() {
...
#[derive(Accounts)]
pub struct FixStoreMaster<'info> {
  #[account(
    mut,
    seeds = [b"store".as_ref(), store.universe.as_ref(), store.creator.as_ref(), &store.slot.to_le_bytes().as_ref()],
    bump
    )]
